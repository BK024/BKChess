...
class Piece(object):
    ...
    def __init__(self, type_, cstr, color, worth, coord, img=0):
        self.L = coord[0]
        self.N = coord[1]
        self.image = pygame.image.load(img) if img else img
        self.rect = self.image.get_rect() if img else 0
    ...
    def moveimg(self):
            ...
            if self.image:
                ...
                self.rect = self.rect.move(L*100, N*100)
...
for player in [[1, 'White', [8, 7]], [-1, 'Black',  [1, 2]]]:
    ...
    C = player[0]
    c = player[1]
    newpiece = Piece('Rook', c, C, 5, (8, R), img='%sRook.png' % c)
    rooks.append(newpiece)
    newpiece = Piece('Rook', c, C, 5, (1, R), img='%sRook.png' % c)
    rooks.append(newpiece)
    newpiece = Piece('Hnight', c, C, 3, (2, R), img='%sKnight.png' % c)
    newpiece = Piece('Hnight', c, C, 3, (7, R), img='%sKnight.png' % c)
    newpiece = Piece('Bishop', c, C, 3, (3, R), img='%sBishop.png' % c)
    newpiece = Piece('Bishop', c, C, 3, (6, R), img='%sBishop.png' % c)
    newpiece = Piece('King', c, C, 0, (5, R), img='%sKing.png' % c)
    kingslist.append(newpiece)
    newpiece = Piece('Queen', c, C, 9, (4, R), img='%sQueen.png' % c)
    rookslist.append(rooks)
    for L in R9:
        newpiece = Piece('Pawn', c, C, 1, (L, Rp), img='%sPawn.png' % c)
